...
import re
import cv2
# from tflite_runtime.interpreter import Interpreter (If you are using the raspberry pi)
...
def load_labels(path='DetectOnCamera/labels.txt'):
...
def main():
    labels = load_labels()
    #Under raspberry pi
    # interpreter = Interpreter('DetectOnCamera\model.tflite')
    interpreter = tf.lite.Interpreter('DetectOnCamera\model.tflite')
